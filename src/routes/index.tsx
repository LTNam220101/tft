...
function HomeComponent() {
    ...
    const emblems = items.filter(i => i.name?.toLowerCase().includes("emblem"));
    ...
    const filteredChamps = champFilter
        ? allChampions.filter(c => c.cost === champFilter)
        : allChampions;
                ...
                <div className="lg:col-span-1 space-y-8 h-fit sticky top-8">
                    <div className="bg-[#16161f] p-6 rounded-2xl border border-white/5 shadow-2xl space-y-8">
                        ...
                        <button
                            ...
                            disabled={loading}
                            className="w-full py-4 bg-gradient-to-br from-amber-400 to-amber-600 text-black font-black text-xl rounded-2xl shadow-xl shadow-amber-500/20 hover:from-amber-300 hover:to-amber-500 transition-all disabled:opacity-50 active:scale-95"
                        >
                    ...
                    {/* Champion Selection */}
                    <div className="bg-[#16161f] p-6 rounded-2xl border border-white/5 shadow-2xl">
                        <section>
                            <div className="flex justify-between items-center mb-4">
                                <h2 className="text-xl font-bold flex items-center gap-2">
                            ...
                            <p className="text-xs text-gray-400 mb-4 px-1">
                                Forced include: {selectedChampIds.length} / {teamSize}
                            </p>

                            <div className="flex flex-wrap gap-1 mb-4">
                                <button
                                    onClick={() => setChampFilter(null)}
                                    className={`px-3 py-1 rounded-full text-[10px] font-bold border transition-all ${champFilter === null ? 'bg-white/10 border-white/20' : 'border-white/5 opacity-50'}`}
                                >
                                    ALL
                                </button>
                                {[1, 2, 3, 4, 5].map(cost => (
                                    <button
                                        key={cost}
                                        onClick={() => setChampFilter(cost)}
                                        className={`px-3 py-1 rounded-full text-[10px] font-bold border transition-all ${champFilter === cost ? 'bg-white/10 border-white/20' : 'border-white/5 opacity-50'}`}
                                    >
                                        ${cost}
                                    </button>
                                ))}
                            ...
                            <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-4 gap-2 max-h-96 overflow-y-auto pr-2 custom-scrollbar">
                                {filteredChamps.map(champ => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`relative p-0.5 rounded-lg transition-all group ${isSelected ? 'ring-2 ring-purple-500 bg-purple-500/10 shadow-lg' : 'opacity-60 hover:opacity-100'}`}
                                            title={champ.name}
                                        >
                                            <img
